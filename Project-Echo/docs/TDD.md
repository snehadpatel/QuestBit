...
- Photon Fusion 2 is the authoritative multiplayer layer.
...
- The host leaves mid-session and the session must be reassigned.
...
The game should avoid relying on each client to independently simulate critical systems such as creature state, objective progression, or facility hazard states. Shared systems should be replicated from a single authoritative source to reduce mismatch and cheating.
...
- What is the minimum set of replicated systems needed for the MVP?
- Should objective state be handled by a general state machine or a dedicated objective manager?
- How much telemetry is necessary before the first public playtest?
- What fallback exists if Photon Fusion or PlayFab services are unavailable during a session?
